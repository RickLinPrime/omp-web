  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        {duration != null && <span style={{ flexShrink: 0 }}>{formatDuration(duration)}</span>}
        {size != null && <span style={{ flexShrink: 0 }}>{formatSize(size)}</span>}
        <span
          title={watching ? t("i18n.liveSync") : t("i18n.notWatching")}
      ...
      >
        <div style={{ width: "min(960px, 100%)", maxHeight: "100%", display: "flex", flexDirection: "column" }}>
          {error && (
            <div style={{ color: "#f87171", fontSize: 13, marginBottom: 12, textAlign: "center" }}>
              {error}
            </div>
...
export function FileViewer({ filePath, cwd, sourceSessionId, onOpenFile, onMentionLines, onAtMention, gitRefreshKey, initialDisplayMode }: Props) {
  ...
  // Video before audio: a <video> element plays audio-only containers too,
  // while an <audio> element cannot show video.
