...
const { MAX_PROJECTED_TREE_DEPTH, buildRollbackEntries, projectTreeForResponse } = await import("./project-tree.ts");
...
test("buildRollbackEntries keeps linear-session user messages that the projected tree drops", () => {
  const tree = [node(
    msg("u1", "user", "第一个问题"),
    [node(msg("a1", "assistant", "第一个回答"), [
      node(msg("u2", "user", "第二个问题"), [node(msg("a2", "assistant", "第二个回答"))]),
    ])],
  )];

  const entries = buildRollbackEntries(tree);
  assert.deepEqual(entries.map((entry) => entry.id), ["u1", "a1", "u2", "a2"]);
  assert.deepEqual(entries.map((entry) => entry.text), ["第一个问题", "第一个回答", "第二个问题", "第二个回答"]);
});

test("buildRollbackEntries flattens every branch with bounded text and no media payload", () => {
  const longText = `a${"x".repeat(400)}`;
  const arm1 = node(msg("u2", "user", "分支一"), [node(msg("a2", "assistant", "答一"))]);
  const arm2 = node(msg("u2b", "user", [{ type: "image", data: "secret-image", mimeType: "image/png" }]), [node(msg("a2b", "assistant", [
    { type: "thinking", thinking: "secret-thinking" },
    { type: "text", text: longText },
  ]))]);
  const root = node(msg("u1", "user", "根问题"), [node(msg("a1", "assistant", "根回答"), [arm1, arm2])]);

  const entries = buildRollbackEntries([root]);
  assert.deepEqual(entries.map((entry) => entry.id), ["u1", "a1", "u2", "a2", "u2b", "a2b"]);
  assert.equal(entries[4].text, "[image]");
  assert.equal(entries[5].text.length, 161);
  assert.equal(entries[5].text.endsWith("…"), true);
  assert.equal(JSON.stringify(entries).includes("secret-image"), false);
  assert.equal(JSON.stringify(entries).includes("secret-thinking"), false);
});

test("buildRollbackEntries skips tool-only assistant turns unless aborted", () => {
  const toolOnly = {
    type: "message",
    id: "a-tool",
    parentId: "u1",
  ...
  const root = node(msg("u1", "user", "问"), [node(toolOnly), node(aborted)]);

  const entries = buildRollbackEntries([root]);
  assert.deepEqual(entries.map((entry) => entry.id), ["u1", "a-aborted"]);
  assert.equal(entries[1].text, "(aborted)");
});